...
const getDoctors = async ( req, res, next) => {
    ...
    if(req.query.specific) {
        ...
        doctors = await DBselect('doctors', '*', "id IN ('" + specifics + "')").catch(err => { sendError({status:400, response:res, message:err}); return false; });
        if(!doctors) return;
    } else {
        doctors = await DBselect('doctors', '*').catch(err => { sendError({status:400, response:res, message:err}); return false; });
        if(!doctors) return;
    }
...
const getDoctor = async ( req, res, next) => {
    ...
    const doctor = await DBselect('doctors', '*', {id: req.params.id}).catch(err => { sendError({status:400, response:res, message:err}); return false; });
    if(!doctor) return;
...
const createDoctor = async ( req, res, next) => {
    ...
    const doctor = await DBinsert('doctors', req.body).catch(err => { sendError({status:400, response:res, message:err}); return false; });
    if(!doctor) return;
...
const updateDoctor = async ( req, res, next) => {
    ...
    const doctor = await DBupdate('doctors', body, {id: req.params.id}).catch(err => { sendError({status:400, response:res, message:err}); return false; });
    if(!doctor) return;
    send(200, res, "success", doctor, ['pass', 'password']);
...
const deleteDoctor = async ( req, res, next) => {
    ...
    const doctor = await DBdelete('doctors', {id: id}).catch(err => { sendError({status:400, response:res, message:err}); return false; });
    if(!doctor) return;
    send(200, res, "success", doctor, ['pass', 'password']);
};
...
const checkDoctor = async (req, res, next) => {
    const doctor = await DBselect('doctors', '*', {id: req.params.id}).catch(err => { sendError({status:400, response:res, message:err}); return false; });
    if(!doctor) return;
    if(doctor.length == 0) sendError({status: 400, response:res, message: "Doctor not found"});
